...
def run_test_draw_points_on_circle():
    ...
    # Test 4:
    title = 'DRAW_POINTS_ON_CIRCLE, test 4:  5 red on blue.'
    window = rg.RoseWindow(400, 400, title)
    circle = rg.Circle(rg.Point(200, 200), 150)
...
def draw_points_on_circle(window, circle, number_of_points, color):
    ...
    for k in range(len(points)):
        circs = rg.Circle(points[k], 10)
        circs.fill_color = color
        circs.attach_to(window)
    window.render()
...
def run_test_pizza():
    ...
    # Test 4:
    title = 'PIZZA test 4:  250 slices, thin (thickness=1) black ' \
            'lines.'
    window = rg.RoseWindow(400, 400, title)
    circle = rg.Circle(rg.Point(200, 200), 150)
...
def run_test_polygon():
    ...
    title = 'POLYGON tests 3:  4 segments with thick black lines, on a lime ' \
            'green circle'
    window = rg.RoseWindow(550, 400, title)
    ...
    polygon(window, circle, 4, 'black', 15)
    window.close_on_mouse_click()
